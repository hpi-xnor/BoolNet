...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels = 16, out_channels = 16, stride = 1, dilation = 1, groups = 1, bias = False, downsample=None, base_width = 64, last_block = False, max_slices = 4):
        ...
        self.conv1 = MultiBConv(self.in_channels, out_channels, 3, stride, dilation, dilation, groups = self.conv_slices_1, bias = bias, wb = True)  
        self.conv2 = MultiBConv(out_channels, out_channels, 3, 1, dilation, dilation, groups = self.conv_slices_2, bias = bias, wb = True)
        
        #self.ghostbnsign1 = GhostBNSign(out_channels, slices = self.max_slices, binary = True)
        #self.ghostbnsign2 = GhostBNSign(out_channels, slices = self.max_slices, binary = True)
    ...
    def forward(self, input):
        ...
        if C//self.in_channels == 2:
          residual = input[:,:,:C//2].contiguous()
        ...
        if not self.last_block:
          ...
          z_1 = OR(z, residual).unsqueeze(2)
            
          if C//self.in_channels == 2:
            z_2 = input[:,:,C//2:].unsqueeze(2)
              
          else:
            z_2 = residual.unsqueeze(2)
          
          #"channel concatenete"
          z = torch.cat((z_1, z_2), dim = 2)
          #"channel shuffle"
          z = z.transpose(2,3).contiguous().view(N, S, -1, H, W).contiguous()
...
class BoolNet(nn.Module):
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        self.ghostbnsign = GhostBNSign(self.inplanes, slices=self.max_slices, binary=False)
            ...
            if self.binary_downsample:
                if stride == 2:
                    downsample = nn.Sequential(
                        BConv(self.inplanes*max_slices*2, planes * block.expansion, kernel_size = 1, stride = 1, padding = 0, groups = 1),
                ...
                if stride == 2:
                    downsample = nn.Sequential(
                        nn.Conv2d(self.inplanes*max_slices*2, planes * block.expansion, kernel_size = 1, stride = 1, padding = 0, groups = max_slices),
...
class boolnet18(BoolNet):
    ...
    """
    def __init__(self, block = BasicBlock, layers =  [2, 2, 2, 2], num_classes = 10, max_slices = 4, binary_downsample = False, replace_stride_with_dilation = None):
        super(boolnet18, self).__init__(block = block, layers = layers, num_classes = num_classes, max_slices = max_slices, binary_downsample = binary_downsample, replace_stride_with_dilation = replace_stride_with_dilation)
...
class boolnet34(BoolNet):
    ...
    """
    def __init__(self, block = BasicBlock, layers =  [3, 4, 6, 3], num_classes = 10, max_slices = 4, binary_downsample = False, replace_stride_with_dilation = None):
        super(boolnet34, self).__init__(block = block, layers = layers, num_classes = num_classes, max_slices = max_slices, binary_downsample = binary_downsample, replace_stride_with_dilation = replace_stride_with_dilation)
