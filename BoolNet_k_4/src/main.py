...
parser.add_argument("--teachers", default = "resnet34,resnet50", type = str, help = "specify the teacher model used in distillation")
...
parser.add_argument("--enlarge_resolution", default = 1, type = int, help = "enlarge the output resolution")
global logger
...
def get_model(opt, num_classes, mode = "student", model = "boolnet18"):
    if mode in {"student", "pretrain"}:
      ...
      if opt.enlarge_resolution == -1:
          replace_stride_with_dilation = [False, False, False] 
      
      elif opt.enlarge_resolution == 1:
          replace_stride_with_dilation = [False, False, True]

      elif opt.enlarge_resolution == 2:
          replace_stride_with_dilation = [False, True, False]

      elif opt.enlarge_resolution == 3:
          replace_stride_with_dilation = [False, True, True]
    
      if model == 'boolnet18':
        net = boolnet18(num_classes = num_classes, max_slices = opt.max_slices, binary_downsample = opt.binary_downsample, replace_stride_with_dilation = replace_stride_with_dilation)
    
      elif model == 'boolnet34':
        net = boolnet34(num_classes = num_classes, max_slices = opt.max_slices, binary_downsample = opt.binary_downsample, replace_stride_with_dilation = replace_stride_with_dilation)
...
def set_device(opt, net, optimizer):  
    if opt.cuda:
      if opt.parallel:
        gpus = [int(i) for i in range(0, opt.parallel)]
